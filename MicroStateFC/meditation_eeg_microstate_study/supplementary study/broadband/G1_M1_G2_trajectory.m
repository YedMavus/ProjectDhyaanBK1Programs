...
featureCsvPath = fullfile(tableDir, sprintf('features_%s_%s_immediateCarryover.csv', bandName, runMode));
writetable(featureTable, featureCsvPath);

save(fullfile(tableDir, sprintf('features_%s_%s_immediateCarryover.mat', bandName, runMode)), ...
    'featureTable', '-v7.3');
...
fprintf('\nRunning statistics for immediate carryover...\n');

features = {'Coverage','Duration','GlobalWPLI'};

comparisons = {
    'G1','M1';   % meditation-state shift from immediate pre-meditation gamma stage
    'M1','G2';   % immediate transition out of meditation
    'G1','G2'    % main immediate carryover: post-meditation gamma vs pre-meditation gamma
};
...
if strcmp(runMode, 'pilot2')
    ...
    writetable(permTable, fullfile(tableDir, sprintf('pilot2_permutation_tests_%s_immediateCarryover.csv', bandName)));

    save(fullfile(tableDir, sprintf('pilot2_permutation_tests_%s_immediateCarryover.mat', bandName)), ...
        'permTable', '-v7.3');
...
elseif strcmp(runMode, 'full10')
    ...
    for f = 1:length(features)
        ...
        for m = 1:K
            ...
            LMMTable.Stage = reordercats(LMMTable.Stage, {'G1','M1','G2'});
            ...
            try
                ...
                save(fullfile(modelDir, sprintf('LMM_%s_MS%s_%s_immediateCarryover.mat', ...
                ...
                writetable(anovaOut, fullfile(tableDir, sprintf('LMM_ANOVA_%s_MS%s_%s_immediateCarryover.csv', ...
                    featName, msName, bandName)));
    ...
    writetable(lmmSummary, fullfile(tableDir, sprintf('LMM_summary_%s_immediateCarryover.csv', bandName)));

    save(fullfile(tableDir, sprintf('LMM_summary_%s_immediateCarryover.mat', bandName)), ...
        'lmmSummary', '-v7.3');
...
fprintf('\nComputing immediate carryover change scores...\n');
...
writetable(changeTable, fullfile(tableDir, sprintf('change_scores_%s_%s_immediateCarryover.csv', bandName, runMode)));

save(fullfile(tableDir, sprintf('change_scores_%s_%s_immediateCarryover.mat', bandName, runMode)), ...
    'changeTable', '-v7.3');
...
fprintf('\nGenerating immediate carryover plots...\n');
...
for f = 1:length(features)
    ...
    for m = 1:K
        ...
        plot_trajectory(featureTable, featName, m, msName, trajectoryStages, ...
            sprintf('%s | MS-%s | G1-M1-G2 immediate carryover trajectory', featName, msName), ...
            fullfile(figDir, sprintf('trajectory_G1_M1_G2_%s_MS%s_%s.png', featName, msName, bandName)));

        plot_window_boxplot(featureTable, featName, m, msName, trajectoryStages, ...
            sprintf('%s | MS-%s | G1-M1-G2 window boxplot', featName, msName), ...
            fullfile(figDir, sprintf('boxplot_G1_M1_G2_%s_MS%s_%s.png', featName, msName, bandName)));

        plot_change_scores(changeTable, featName, m, msName, ...
            sprintf('%s | MS-%s | immediate carryover change scores', featName, msName), ...
            fullfile(figDir, sprintf('changescore_G1_M1_G2_%s_MS%s_%s.png', featName, msName, bandName)));
...
function plot_trajectory(T, featName, msID, msName, stages, figTitle, savePath)
    ...
    for g = 1:length(groups)
        ...
        for s = 1:length(subjects)
            ...
            plot(1:length(stages), y, '-o', ...
                'DisplayName', sprintf('%s-%s', groupName, subjects{s}));
        end
    end
