...
% 5. Please update/change the paths to folders and files accordingly to run the code
%% ============================================================
...
outputRoot = fullfile('/Study2_StateSpecific_Alpha_Results_New', runMode);
...
for s = 1:length(subjects)
    ...
    for st = 1:length(allStages)
        ...
        data2D = preprocess_alpha_CAR_once(data2D, Fs);
...
function data = preprocess_alpha_CAR_once(data, Fs)

    validTime = all(~isnan(data), 1);
    data = data(:, validTime);

    % Demean each channel; this is NOT CAR
    data = data - mean(data, 2);

    % First broadband filter
    ...
    % Alpha band extraction: no CAR after this
