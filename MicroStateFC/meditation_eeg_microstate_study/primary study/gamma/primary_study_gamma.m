%% CODE FOR Alpha band case: STUDY 2
...
outputRoot = fullfile('/Users/anatapmitra/Desktop/NSP project/Study2_StateSpecific_Gamma_Results', runMode);
...
fprintf('\nSubjects used for Study 2 alpha:\n');
disp(table(subjects, groupLabels));

%% ============================================================
% 1. LOAD SUBJECT TEMPLATES AND DERIVE COMMON ALPHA TEMPLATES
%% ============================================================

fprintf('\nLoading saved alpha subject templates...\n');
...
for s = 1:length(subjects)
    ...
    if ~exist(templatePath, 'file')
        error('Missing alpha template file:\n%s', templatePath);
    end
...
fprintf('All alpha subject template matrix: [%d x %d]\n', ...
    size(allSubjectTemplates,1), size(allSubjectTemplates,2));

fprintf('Deriving common K=5 alpha templates...\n');
...
fprintf('\nBackfitting and extracting alpha features...\n');
...
for s = 1:length(subjects)
    ...
    for st = 1:length(allStages)
        ...
        data2D = preprocess_gamma(data2D, Fs);
...
fprintf('\nComputing alpha microstate dominance patterns...\n');
...
fprintf('\nGenerating Study 2 alpha plots...\n');
...
for f = 1:length(features)
    ...
    plot_group_delta_heatmap(groupStatsTable, featName, allStages, microstateNames, ...
        sprintf('Study 2 alpha | %s | Meditator - Control', featName), ...
...
plot_stage_profile(subjectFeatureTable, 'Coverage', 'M1', microstateNames, ...
    'Study 2 alpha | M1 Coverage profile', ...
...
%
function data = preprocess_gamma(data, Fs)
    ...
    % Gamma band
    % Using 30-45 Hz because your broadband cleaning cuts at 45 Hz
    [b2, a2] = butter(4, [30 45] / (Fs/2), 'bandpass');
    data = filtfilt(b2, a2, data')';

    % Common average reference again
    data = data - mean(data, 1);
...
function plot_dominance_pattern(dominanceTable, stages, savePath)
    ...
    title('Study 2 alpha | Dominant microstate by group and stage');
