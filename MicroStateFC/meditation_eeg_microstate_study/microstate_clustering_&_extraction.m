%% CODE FOR MICORSTATE CLUSTERING AND EXTRACTION ALGORTIHTM 
clc; clear; close all;
...
% Please change the folder paths accordingly for running this code
%% ============================================================
...
outputRoot = "/MicroStateTemplates_fastK5_v3";
...
function data = preprocess_eeg_for_microstates(data, Fs, bandName)
    ...
    % Remove time samples where any channel is NaN
    data = data(:, all(~isnan(data), 1));

    % Channel-wise demean / DC offset removal.
    % This is NOT CAR; it centers each channel over time.
    data = data - mean(data, 2);

    % 0.5-45 Hz broadband filtering first
    [b1, a1] = butter(4, [0.5 45] / (Fs/2), 'bandpass');
    data = filtfilt(b1, a1, data')';

    % CAR ONLY ONCE: after 0.5-45 Hz filtering
    % Do not apply CAR before this or after alpha/gamma filtering.
    data = data - mean(data, 1);

    % Optional band-specific filtering only. No CAR after this.
...
function desc = get_preprocessing_description(bandName)
    ...
    desc.step3 = 'Channel-wise demean / remove channel-specific DC offset';
    desc.step4 = 'Butterworth 4th-order 0.5-45 Hz bandpass';
    desc.step5 = 'Common average reference applied once after 0.5-45 Hz filtering';
    desc.step6 = 'Optional second band-specific filter only, without additional CAR';
    desc.step7 = 'GFP peak extraction';
    desc.step8 = 'Keep strongest GFP peaks up to maxGFPMapsPerStage';
    desc.step9 = 'GFP map normalization';
